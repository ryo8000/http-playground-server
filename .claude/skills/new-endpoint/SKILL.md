...
1. **Service** — `src/services/<name>.ts`: pure functions taking plain values (never `req`/`res`). Apply the layering rule (`.claude/rules/code-style.md` → Layering) to decide what the service returns vs what the route orchestrates. Skip this file only if the route has no logic at all (like `/uuid`).
...
- **HEAD** — streaming/long-running endpoints must short-circuit after headers (`if (req.method === 'HEAD') { res.end(); return; }`; see `src/routes/drip.ts`) so no timer/stream runs; guard body assertions with `if (method !== 'head')`.
- **Constants** — per-endpoint limits/defaults stay hardcoded; only cross-cutting resource bounds go in `env.ts` (like `MAX_DELAY`). Comment only non-obvious values (`8192` = 8 KiB header limit).
- **Socket/res guards** — add `socket?.`/`res.destroyed` checks only after confirming the failure actually occurs; `res.on('close')` already clears timers on disconnect.
- **E2E limits** — endpoints that break the connection, never respond, or fail client-side can't be asserted in Newman; omit them.

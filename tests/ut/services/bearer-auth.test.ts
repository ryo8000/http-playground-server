...
describe('bearerAuth', () => {
  ...
  it.each([
    ...
    { header: 'Bearer', reason: 'a Bearer scheme without a trailing space' },
  ...
  it.each([
    { header: 'Bearer wrong-token', reason: 'does not match' },
    { header: 'Bearer ', reason: 'is empty after the scheme' },
  ])('should return 401 when the token $reason', ({ header }) => {
    const result = bearerAuth('secret-token', header);
    expect(result).toEqual({
      status: 401,

...
describe('errorRouter', () => {
  ...
  describe.each(httpMethods)('%s method', (method) => {
    ...
    it('should cause connection reset for "network" type', async () => {
      const response = await request(app)[method]('/error/network').catch((err) => err);
      expect(response.message).toMatch(/socket hang up|ECONNRESET/);
    });

    it('should timeout for "timeout" type', async () => {
      const timeoutMs = 1000;
      const response = await request(app)[method]('/error/timeout').timeout({ deadline: timeoutMs }).catch((err) => err);
      expect(response.timeout).toBeTruthy();
    });

    it('should return malformed JSON for "malformed-json" type', async () => {
      const response = await request(app)[method]('/error/malformed-json').catch((err) => {
        if (method !== 'head') {
    ...
    it('should return 400 for unsupported type', async () => {
      ...
      if (method !== 'head') {
        expect(response.body).toEqual({
          ...
          message: 'Bad Request',
          error: {
            message: "Invalid error type. SupportedTypes are 'timeout', 'network', 'malformed-json' and 'error'",
          }
        });
      }
    });
